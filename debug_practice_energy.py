# -*- coding: utf-8 -*-
# ============================================================
#  debug_practice_energy.py  --  调试器练习副本，不参与交付
#  2026-09-07
# ============================================================
#  这是 lesson04_energy.py 在 2026-09-07 的一份副本，唯一用途是练 VS Code 调试器
#  （断点 / 单步 / 变量面板 / 条件断点）。主线代码是 lesson04_energy.py，不是这个文件。
#
#  和主线的两处差别：
#    1. 统计量在这里是手写的（mean/var/se 三段、卡方循环、表头 print）。
#       主线在 2026-09-08 把这些抽进了 mcstat.py，这份副本没有跟进。
#    2. 规模砍小了：N=20000 / N_hist=2000 / N_hist_B=1000
#       （主线是 1000000 / 50000 / 20000）。断点单步的前提是它得几秒钟跑完。
#
#  【练习内容 —— 已完成，bug 已改回】
#  方法是把 Ex2(alpha) 在 alpha==0 分支的返回值故意改成 0（正确值是 2），
#  然后不靠 print，只用断点把它抓出来。
#
#  抓法：程序停在  assert abs(n_mean - Renew) < 3*n_se 。变量面板里
#      n_mean = 19.193        <- 模拟测出来的平均碰撞数
#      Renew  = 18.19753719
#      Naive  = 18.19753719   <- 和 Renew 逐位相等，这就是烟枪
#  Renew - Naive = E[x^2] / (2*xi^2) = 超越量，取值在 [2/3, 1]，严格为正，
#  物理上不可能等于 0。两个数一旦逐位相等，只有一种解释：分子里那一项被吃掉了。
#  当时的偏差是 +7.47 sigma。
#
#  这条 assert 之所以能抓住它，是因为它有两个独立算出来的量：Renew 是判据，
#  Naive 是免费摆在同一张表同一行上的独立参照物。
#
#  Ex2 现在是正确的（alpha==0 时 return 2），所以本文件跑完退出码 0，九条验证全过。
#  想重做这个练习：把 Ex2 里 alpha==0 分支的 return 2 改成 return 0。
# ============================================================
# 写完这一段，扫一遍：
