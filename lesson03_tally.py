...
def print_flux_table(track_sum, track_sq, N, dz, NL, title):
    ...
    for k in range(NL):
        z_lo = k * dz
        z_hi = z_lo + dz
        phi = track_sum[k]/(N*dz)          # 平均径迹 / 体积，你有公式
        err = math.sqrt(track_sq[k]/(N-1)/N-(phi*dz)**2/(N-1))/dz          # 标准误 / 体积
...
mean2=v_sum2/N
err_t2=math.sqrt(v_sq2/(N-1)/N-(v_sum2/N)**2/(N-1))
...
mean3=v_sum3/N
err_t3=math.sqrt(v_sq3/(N-1)/N-(v_sum3/N)**2/(N-1))
